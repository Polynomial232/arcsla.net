  ...
  return (
    <nav
      className={cn(
        ...
        scrolled
          ? "bg-white/80 backdrop-blur-md border-b border-accent-purple/20 py-2"
          : "bg-transparent",
      )}
    >
...
const SectionHeader = ({
  ...
  <div className="text-center mb-16 px-4">
    ...
    <h2 className="text-4xl md:text-5xl font-sans font-black mb-4 uppercase tracking-tighter text-deep-purple">
      {title}
    </h2>
    {subtitle && (
      <p className="text-muted-foreground max-w-2xl mx-auto text-lg italic">{subtitle}</p>
    )}
...
export default function Home() {
  ...
  const eventStatus = featuredEvent
    ? getEventStatus(featuredEvent.date_start, featuredEvent.date_end)
    : null;

  return (
    <div className="relative min-h-screen bg-background text-foreground selection:bg-accent-yellow selection:text-deep-purple">
      <SEO
        title="Home of the Sovereigns"
        description="Portal resmi PT Arcsla Cakrawala Indonesia — Mengelola talent, produksi YouTube, fanart, dan komunitas kreator digital Indonesia."
          ...
          itemListElement: [
            {
              ...
              position: 1,
              name: "Home",
              item: "https://arcsla.net",
            },
            {
              "@type": "ListItem",
              position: 2,
              name: "Gallery",
              item: "https://arcsla.net/gallery",
            },
          ],
        }}
      />
                  ...
                  <div className="aspect-square rounded-3xl overflow-hidden border-4 border-white shadow-xl relative">
                    ...
                    <div
                      className={cn(
                        ...
                        eventStatus === "Active Gathering"
                          ? "bg-accent-purple text-white animate-pulse"
                          : "bg-accent-yellow text-deep-purple",
                      )}
                    >
                      {eventStatus}
                    </div>
          ...
          <div className="container mx-auto">
            <Carousel
              items={teamMembers.map((member: any, index: number) => (
                <TestimonialCard key={member.id} testimonial={member} index={index} />
              ))}
            />
          </div>
        </div>
      ...
      <section id="about" className="py-24 relative">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center gap-16">
          ...
          <div className="w-full md:w-1/2 space-y-6">
            <h2 className="text-5xl font-sans font-black text-deep-purple uppercase leading-tight">
              Origins of the <br />
              Digital Fortress
            </h2>
            <p className="text-xl text-muted-foreground italic leading-relaxed">
              In an era of fleeting trends and hollow echoes, ARCSLA was forged from the fires of
              passion and the steel of innovation. We are not merely a company; we are a kingdom
              dedicated to the pursuit of digital mastery.
            </p>
            <p className="text-muted-foreground">
              Our quest began with the vision of uniting creators under a single banner of
              excellence. From the majestic heights of YouTube production to the deep symphonies of
              musical distribution, we build legacies that endure through the ages.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          ...
          >
            {youtubeVideos.map((id: string) => (
              <div
                key={id}
                className="aspect-video rounded-xl overflow-hidden border border-accent-purple/10 shadow-lg group hover:border-accent-purple/30 transition-all"
              >
                <iframe
      ...
      >
        ...
        <motion.div style={{ y: joinBackgroundY }} className="absolute inset-0 z-0 scale-110">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          >
            ...
            <p className="text-white/90 text-xl font-medium leading-relaxed mb-10 italic">
              "Every contribution strengthens our walls and sharpens our blades for the next
              creative conquest in the digital realm."
            </p>
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="text-center pt-8 border-t border-accent-purple/5 w-full">
              <p className="text-xs font-black uppercase tracking-[0.3em] text-muted-foreground">
                © {new Date().getFullYear()} {import.meta.env.VITE_COMPANY_NAME || "ARCSLA"}. All
                rights reserved by the Crown.
              </p>
              <p className="mt-2 text-[10px] italic text-accent-purple/40">
                "In Digital Sovereignty We Trust"
              </p>
            </div>
          </div>
        </div>
