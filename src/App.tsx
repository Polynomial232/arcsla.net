...
import { Menu, X, Crown, Youtube, Music, Heart, Mail, Disc, Share2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { NAV_LINKS, TEAM_MEMBERS, YOUTUBE_VIDEOS } from "./data";
...
const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
  if (href.startsWith("#")) {
    ...
    if (targetElement) {
      ...
      // Clean URL after scroll has likely finished
      setTimeout(() => {
...
export default function App() {
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-accent-yellow selection:text-deep-purple">
          ...
          <div className="w-full md:w-1/2 space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              <div className="flex flex-col">
                <span className="text-3xl font-sans font-black text-accent-purple">150+</span>
                <span className="text-xs uppercase tracking-widest opacity-50 font-bold">Glorious Victories</span>
              </div>
              <div className="w-px h-12 bg-accent-yellow" />
              <div className="flex flex-col">
                <span className="text-3xl font-sans font-black text-accent-purple">5M+</span>
                <span className="text-xs uppercase tracking-widest opacity-50 font-bold">Subjects Reached</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Donation Section */}
      <section id="donate" className="py-24 bg-pastel-purple/20 relative">
        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-5" />
        <SectionHeader
          title="Fortify the Kingdom"
          subtitle="Support our noble quests and help us expand the boundaries of our digital realm."
          icon={Heart}
        />
        <div className="container mx-auto px-6 text-center">
          <div className="max-w-xl mx-auto p-12 bg-white border border-accent-purple/20 rounded-3xl shadow-2xl backdrop-blur-sm">
            <div className="mb-8">
              <div className="w-20 h-20 bg-accent-yellow/20 rounded-full flex items-center justify-center mx-auto mb-6">
                <Heart className="w-10 h-10 text-deep-purple fill-deep-purple" />
              </div>
              <h3 className="text-2xl font-sans font-black text-deep-purple uppercase mb-4">Tribute of Support</h3>
              <p className="text-muted-foreground">Every contribution strengthens our walls and sharpens our blades for the next creative conquest.</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button className="px-8 py-4 bg-accent-yellow text-deep-purple font-bold uppercase tracking-widest rounded-full hover:bg-black hover:text-white transition-all shadow-[0_5px_15px_rgba(253,224,71,0.4)] active:scale-95">
                Donate Now
              </button>
              <button className="px-8 py-4 border-2 border-deep-purple text-deep-purple font-bold uppercase tracking-widest rounded-full hover:bg-deep-purple/10 transition-all active:scale-95">
                Explore Patreon
              </button>
            </div>
          </div>
